...
for iter in range(5):
    ...
    for i in range(len(plainvars)): #ignore the new auxiliary variables of disjvars
         plainvars[i].Obj = objnew[i]
         disjvars[i].Obj = objnew[i]

    if args.optimal:
        optimalmodel = plainmodel.copy()
        optimalmodel.resetParams()
    ...
    print("INFO",modelname+"_costn"+str(iter),len(nodedisjunctions),plainmodel.NodeCount,disjmodel.NodeCount,plainmodel.MIPGap,disjmodel.MIPGap,plainmodel.Runtime,disjmodel.Runtime)
